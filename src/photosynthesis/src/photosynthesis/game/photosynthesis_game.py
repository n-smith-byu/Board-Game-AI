...
from ..game.game_state import PlayerGameState
...
if TYPE_CHECKING:
    from board_game import Player

class PhotosynthesisGame(bg.BoardGame):
    _ai_player_cls = None
    _human_player_cls = None
    ...
    @classmethod
    def get_ai_player_class(cls) -> Type[bg.AIPlayer]:
        if not cls._ai_player_cls:
            from ..players import RandomPlayer      # default player class is RandomPlayer
            cls._ai_player_cls = RandomPlayer

        return cls._ai_player_cls
    
    @classmethod
    def set_ai_player_class(cls, new_class):
        cls._ai_player_class = new_class
    
    @classmethod
    def get_human_player_class(cls) -> type[bg.HumanPlayer]:
        if not cls._human_player_cls:
            from ..players import HumanPlayer
            cls._human_player_cls = HumanPlayer
            
        return cls._human_player_cls

    SUN_POSITIONS = 6

    def __init__(self, players: list[bg.Player], 
